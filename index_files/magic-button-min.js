function S123MagicButton(settings) {
  ...
  function buildButton() {
    ...
    $(document).trigger('S123MagicButton.initialized');
  }
  ...
  function getOptionActionAttrByType(option) {
    ...
    if (_.type === 'contact') {
      ...
      action = action.replace(new RegExp('\\{{description}}', 'gm'), removeNewLines($('meta[name="description"]').attr('content')));
  ...
  function addBottomMenuHandler(isMobile) {
    if (!isMobile && IsHomepage() && _.menuLayoutNUM == '15') {
      if ($(window).scrollTop() === 0) _.$container.hide();
      $(window).off('scroll.S123MagicButton').on('scroll.S123MagicButton', function() {
        if ($(window).scrollTop() > _.$mainNav.outerHeight()) {
    ...
    $(document).off('previewScale.deviceTypeChange').on('previewScale.deviceTypeChange', function(event, deviceType) {
    ...
    generateHtml: function() {
      ...
      if (hasLogo) {
        ...
        if (this.siteLogo.indexOf('site123-image-icon') != -1) {
          html += '<i class="fa ' + S123.escapeHtml(this.siteLogo) + '"></i>';
        } else if (this.siteLogo.indexOf('.svg') != -1) {
          html += '<div class="svg-m" style="mask: url(\'' + this.siteLogo.replace('400_', 'normal_') + '\'); -webkit-mask: url(\'' + this.siteLogo.replace('400_', 'normal_') + '\');"></div>';
        } else {
      ...
      html += '<p>' + S123.escapeHtml(this.data.value) + '</p>';
