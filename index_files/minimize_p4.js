...
function DonateModuleInitialize_Layout1() {
  $(document).on('s123.page.ready', function(event) {
    ...
    $section.each(function(index) {
      ...
      var $donateButtons = $sectionThis.find('.donate-btn');
      var $donateBtnOther = $sectionThis.find('.donate-btn-other');
      var $donateAmounts = $sectionThis.find('.donate-amount');
      $donateButtons.off('click').on('click', function() {
        var $btn = $(this);
        var $donatePreview = $sectionThis.find('.donation-price-preview span[data-type="price"]');
        setActiveBtn($donateButtons, $donateBtnOther, $btn);
        $sectionThis.find('.donate-price-input-container').css({
          visibility: 'hidden'
        }).removeClass('shown');
        $donateAmounts.val($.trim($(this).data('amount')));
        $(document).trigger('donate_price_update', [$donateAmounts]);
        UpdateAmount($donatePreview, $btn.data('amount'));
      });
      $donateButtons.eq(1).trigger('click');
      $donateBtnOther.off('click').on('click', function() {
        var $btn = $(this);
        setActiveBtn($donateButtons, $donateBtnOther, $btn);
        $sectionThis.find('.donate-price-input-container').css({
          visibility: 'visible'
      ...
      $donateAmounts.off('input').on('input', function(event) {
        var $donatePreview = $sectionThis.find('.donation-price-preview span[data-type="price"]');
        var eventKey = event.which;
        clearTimeout($donateAmounts.inputFinished);
        $donateAmounts.inputFinished = setTimeout(function() {
          if (!$donateAmounts.val() || eventKey === 13) return;
          if (!$donateForm.valid()) return;
          UpdateAmount($donatePreview, $donateAmounts.val());
        }, 500);
      });
      $donateForm.validate({
        errorElement: 'div',
        ...
        submitHandler: function(form) {
          ...
          $('<form action="/versions/' + $('#versionNUM').val() + '/wizard/orders/front/addToCart.php" method="post">' + '<input type="text" name="w" value="' + $('#w').val() + '" />' + '<input type="text" name="websiteID" value="' + $('#websiteID').val() + '" />' + '<input type="text" name="uniquePageID" value="' + formValues.uniquePageID + '" />' + '<input type="text" name="moduleID" value="' + formValues.moduleID + '" />' + '<input type="text" name="tranW" value="' + formValues.tranW + '" />' + '<input type="text" name="amount" value="1" />' + '<input type="text" name="dynamicPrice" value="' + formValues.amount + '" />' + '<input type="text" name="redirect" value="1" />' + '</form>')
            .appendTo('body')
            .submit();
          return false;
        }
  ...
  function setActiveBtn($donateButtons, $donateBtnOther, $btn) {
    $donateButtons.removeClass('selectedPrice');
    $donateBtnOther.removeClass('selectedPrice');
    $donateButtons.removeClass('btn-primary');
    $donateBtnOther.removeClass('btn-primary');
    $btn.addClass('selectedPrice');
    $btn.addClass('btn-primary');
  }

  function UpdateAmount($donatePreview, amount) {
    $donatePreview.countTo({
      from: 0,
      to: amount,
      speed: 1000,
      decimals: amount.toString().indexOf('.') === -1 ? 0 : 2,
      refreshInterval: 50,
